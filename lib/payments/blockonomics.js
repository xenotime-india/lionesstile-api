      ...
      if (amount >= order.orderExpectedBtc) {
        try {
          ...
          sendEmail(
            ...
            `Your payment with ${config.cartTitle}`,
...
router.post('/checkout_action', (req, res, next) => {
  ...
  axios
    ...
    .then((response) => {
      blockonomicsParams.expectedBtc =
        Math.round((req.session.totalCartAmount / response.data.price) * Math.pow(10, 8)) /
        Math.pow(10, 8);
      // get new address
      axios
        .post(
          blockonomicsConfig.hostUrl + blockonomicsConfig.newAddressApi,
          {},
          {
            headers: {
              ...
              Authorization: `Bearer ${blockonomicsConfig.apiKey}`,
            },
          },
        )
        .then((response) => {
