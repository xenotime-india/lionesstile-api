...
const updateTotalCart = async (req, res) => {
  ...
  config.modules.loaded.shipping.calculateShipping(req.session.totalCartNetAmount, config, req);
  ...
  req.session.totalCartAmount =
    req.session.totalCartNetAmount - req.session.totalCartDiscount + req.session.totalCartShipping;
};
