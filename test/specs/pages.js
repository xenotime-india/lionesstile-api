...
test('[Success] Create a new page', async (t) => {
  // Login
  await g.request.post('/admin/login_action').send({
    email: g.users[0].userEmail,
    password: 'test',
  });
  // Add page
  const res = await g.request.post('/admin/settings/page').send({
...
test('[Success] Update a page', async (t) => {
  ...
  // Update page
  const res = await g.request.post('/admin/settings/page').send(updatedPage);
...
test('[Success] Delete a page', async (t) => {
  ...
  // Update page
  const res = await g.request.post('/admin/settings/page/delete').send({
    pageId: insertedPageId,
  });
