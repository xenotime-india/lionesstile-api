...
test('[Success] Update cart', async (t) => {
  const cart = await g.request.get('/cart/retrieve').expect(200);
  ...
  const checkCart = await g.request.get('/cart/retrieve').expect(200);

  // Check new quantity and total price has been updated
  t.deepEqual(checkCart.body.cart[cartId].quantity, 10);
  t.deepEqual(
    checkCart.body.cart[cartId].totalItemPrice,
    cart.body.cart[cartId].totalItemPrice * 10,
  );
});
