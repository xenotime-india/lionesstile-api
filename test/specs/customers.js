...
test('[Success] Create a customer', async (t) => {
  ...
  const res = await g.request.post('/customer/create').send(customer).expect(200);
...
test('[Success] Get customer list', async (t) => {
  const res = await g.request.get('/admin/customers').set('apiKey', g.users[0].apiKey).expect(200);
