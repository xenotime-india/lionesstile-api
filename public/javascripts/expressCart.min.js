function checkMaxQuantity(t, a) {
  if ($('#maxQuantity').length) {
    ...
    if (parseInt($(t.target).val()) > parseInt($('#maxQuantity').val())) {
      ...
      t.preventDefault(),
        a.val(e.slice(0, -1)),
        showNotification(`Exceeds maximum quantity: ${$('#maxQuantity').val()}`, 'warning', !1);
    }
  }
}
...
function updateCart(t) {
  $.ajax({
    ...
    data: { cartId: t.attr('data-cartid'), productId: t.attr('data-id'), quantity: t.val() },
  })
    .done(function (t) {
      updateCartDiv();
    })
  ...
  $.ajax({ method: 'GET', url: '/checkout/cartdata' })
    .done(function (t) {
      var a = t.cart,
        ...
        r = `${e.shippingMessage} :<strong id="shipping-amount">${t.currencySymbol}${i}</strong>`;
      0 === e.totalCartShipping && (r = `<span id="shipping-amount">${e.shippingMessage}</span>`);
          ...
          ? ($('#cart-empty').empty(),
            Object.keys(a).forEach(function (e) {
              var o = a[e],
                ...
                r = '';
              o.variantId && (r += `<strong>Option:</strong> ${o.variantTitle}`);
              var c = `<img class="img-fluid" src="/uploads/placeholder.png" alt="${o.title} product image"></img>`;
              o.productImage &&
      ...
      a
        ? ($('.cartTotalsWrapper').html(d), $('.cart-buttons').removeClass('d-none'))
...
function emptyCart() {
  $.ajax({ method: 'POST', url: '/product/emptycart' }).done(function (t) {
...
$(document).ready(function () {
  if (
    ($(window).width() < 768 &&
      ...
      0 === $('#navbar ul li').length && $('#navbar').hide(),
      $('#offcanvasClose').hide()),
    $('#userSetupForm')
      .validator()
  ...
  if (
    ($('#customerLogout').on('click', function (t) {
      $.ajax({ method: 'POST', url: '/customer/logout', data: {} }).done(function (t) {
        location.reload();
      });
    }),
    $('#customerForgotten')
    ...
    $('#checkoutInformation')
      ...
      .on('click', function (t) {
        if (
          (t.preventDefault(),
          0 === $('#shipping-form').validator('validate').has('.has-error').length)
        ) {
          var a = '/customer/save';
          $('#createAccountCheckbox').prop('checked') && (a = '/customer/create'),
            $.ajax({
              method: 'POST',
              url: a,
              data: {
    ...
    $('#removeDiscountCode').on('click', function (t) {
      t.preventDefault(),
        $.ajax({ method: 'POST', url: '/checkout/removediscountcode', data: {} })
          .done(function (t) {
            showNotification(t.message, 'success', !0);
          })
          .fail(function (t) {
    ...
    $('#customerloginForm').on('click', function (t) {
      t.isDefaultPrevented() ||
        (t.preventDefault(),
        $.ajax({
          method: 'POST',
          url: '/customer/login_action',
          data: { loginEmail: $('#email').val(), loginPassword: $('#password').val() },
        })
          .done(function (t) {
    ...
    $(document).on('change', '#product_variant', function (t) {
      var a = $(this).find(':selected').attr('data-price'),
        e = $('#currencySymbol').val();
    ...
    $(document).on('click', '.product-add-to-cart', function (t) {
      parseInt($('#product_quantity').val()) < 1 && $('#product_quantity').val(1),
        $.ajax({
          method: 'POST',
          url: '/product/addtocart',
          data: {
    ...
    $(document).on('click', '.add-to-cart', function (t) {
      ...
      $(this).attr('data-link') && (a = '/product/' + $(this).attr('data-link')),
        'true' === $(this).attr('data-has-variants')
          ? (window.location = a)
          : $.ajax({
              method: 'POST',
              url: '/product/addtocart',
  ...
  if ($('#blockonomics_div').length > 0) {
    var d = $('#blockonomics_div').data('orderid') || '',
      l = $('#blockonomics_div').data('timestamp') || -1,
      u = $('#blockonomics_div').data('address') || '',
      m = new WebSocket('wss://www.blockonomics.co/payment/' + u + '?timestamp=' + l);
    m.onopen = function (t) {};
    setTimeout(function () {
    ...
    var p = $('#blockonomics_timeout'),
      f = new Date(new Date().getTime() + 6e5),
      v = setInterval(function () {
        var t = new Date().getTime(),
          a = f - t;
        if (a < 0) clearInterval(v);
        else {
          var e = Math.floor((a % 36e5) / 6e4),
    ...
    m.onmessage = function (t) {
      ...
      if (0 === a.status || 1 === a.status || 2 === a.status) {
        var e = '<br>View <b><a href="/payment/' + d + '">Order</a></b>';
        $('#blockonomics_waiting').html('Payment detected (<b>' + a.value / 1e8 + ' BTC</b>).' + e),
