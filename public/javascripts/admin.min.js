function globalSearch() {
  $('#global-search-results').empty(),
    $.ajax({
      type: 'POST',
      url: '/admin/searchall',
      data: { searchValue: $('#global-search-value').val() },
    }).done((e) => {
      $('#global-search').html('<i class="feather" data-feather="search"></i>');
      let t = !1;
      if (
        (e.customers.forEach((e) => {
          t = !0;
        ...
        e.orders.forEach((e) => {
          ...
          const a = `\n            <li class="list-group-item global-result text-center" data-url="/admin/order/view/${
            ...
            e.orderLastname
          }</div>\n                    <div class="col global-result-detail gr-click">${moment(
            e.orderDate,
          ).format(
            'YYYY/MM/DD',
          )}</div>\n                    <div class="col global-result-detail gr-click">${
        ...
        e.products.forEach((e) => {
          t = !0;
          const a = `<li class="list-group-item global-result text-center" data-url="/admin/product/edit/${
            e._id
          }">\n                <div class="row">\n                    <div class="col global-result-type gr-click"><i class="feather" data-feather="tag"></i> Product</div>\n                    <div class="col global-result-detail gr-click">${
            e.productTitle
          }</div>\n                    <div class="col global-result-detail gr-click">${$(
            '#currencySymbol',
          ).val()}${numeral(e.productPrice).format(
            '0.00',
          )}</div>\n                </div>\n            </li>`;
          $('#global-search-results').append(a);
        }),
...
function validationErrors(e) {
  ...
  return (
    e.forEach((e) => {
      t += `<p>${e.dataPath.replace('/', '')} - <span class="text-danger">${e.message}<span></p>`;
    }),
    t
  );
}
$(document).ready(function () {
  if (
    ($.ajaxSetup({ headers: { 'csrf-token': $('meta[name="csrfToken"]').attr('content') } }),
    $(document).on('click', '#btnGenerateAPIkey', function (e) {
      e.preventDefault(),
        $.ajax({ method: 'POST', url: '/admin/createApiKey' })
          .done(function (e) {
          ...
          $.ajax({
            ...
            .fail(function (e) {
              if (e.responseJSON.length > 0) {
                var t = validationErrors(e.responseJSON);
                return $('#validationModalBody').html(t), void $('#validationModal').modal('show');
              }
              showNotification(e.responseJSON.message, 'danger');
            }));
      }),
            ...
            .fail(function (e) {
              if (e.responseJSON.length > 0) {
                ...
                return (
                  ...
                  $('#validationModalBody').html(t),
                  void $('#validationModal').modal('show')
                );
              }
              showNotification(e.responseJSON.message, 'danger');
            }));
    ...
    $(document).on('click', '#lookupCustomer', function (e) {
      e.preventDefault(),
        $.ajax({
          ...
          .done(function (e) {
            showNotification(e.message, 'success'),
              $('#orderFirstName').val(e.customer.firstName),
              $('#orderLastName').val(e.customer.lastName),
              $('#orderAddress1').val(e.customer.address1),
              $('#orderAddress2').val(e.customer.address2),
              $('#orderCountry').val(e.customer.country),
              $('#orderState').val(e.customer.state),
              $('#orderPostcode').val(e.customer.postcode),
              $('#orderPhone').val(e.customer.phone);
          })
          .fail(function (e) {
            showNotification(e.responseJSON.message, 'danger');
          });
    ...
    $(document).on('click', '#orderCreate', function (e) {
      e.preventDefault(),
        0 === $('#createOrderForm').validator('validate').has('.has-error').length &&
          $.ajax({
            method: 'POST',
            ...
            .done(function (e) {
              showNotification(e.message, 'success'),
                (window.location = `/admin/order/view/${e.orderId}`);
            })
            .fail(function (e) {
              showNotification(e.responseJSON.message, 'danger');
            });
    ...
    $('#sendTestEmail').on('click', function (e) {
      e.preventDefault(),
        $.ajax({ method: 'POST', url: '/admin/testEmail' })
          .done(function (e) {
    ...
    $('#updateCustomer')
      ...
      .on('click', function (e) {
        e.preventDefault(),
          0 === $('#customer-form').validator('validate').has('.has-error').length &&
            $.ajax({
              method: 'POST',
  ...
  $(document).on('click', '#btnPageUpdate', function (e) {
          ...
          $.ajax({
            ...
            .done(function (e) {
              showNotification(
                ...
                'success',
                !1,
                '/admin/settings/discount/edit/' + e.discountId,
              );
            })
            .fail(function (e) {
              showNotification(e.responseJSON.message, 'danger');
      ...
      e.preventDefault(),
        confirm('Are you sure?') &&
          $.ajax({
            ...
            data: { discountId: $(this).attr('data-id') },
          })
            .done(function (e) {
              showNotification(e.message, 'success', !0);
            })
    ...
    $(document).on('click', '#settings-menu-new', function (e) {
      e.preventDefault(),
        $.ajax({
          ...
          data: { navMenu: $('#newNavMenu').val(), navLink: $('#newNavLink').val() },
        })
          .done(function (e) {
            showNotification(e.message, 'success', !0);
          })
      ...
      $('#draggable_list').sortable({
        update: function () {
          ...
          $('.navId').each(function (t) {
            ...
            $.ajax({ data: { order: e }, type: 'POST', url: '/admin/settings/menu/saveOrder' })
              .done(function () {
    ...
    $(document).on('click', '#uploadButton', function (e) {
      ...
      t.append('productId', $('#productId').val()),
        $.ajax({
          method: 'POST',
    ...
    $('#global-search-value').on('keyup', (e) => {
      '' === $('#global-search-value').val() &&
        ($('#global-search-results').empty(), $('#global-search-results').addClass('invisible'));
      let t = 3;
      /^\d*\.?\d*$/.test($('#global-search-value').val()) && (t = 1),
        $('#global-search-value').val().length > t &&
          ($('#global-search').html('<span class="fa fa-spinner fa-spin"></span>'), globalSearch());
    }),
    ...
    $('body').on('click', '.gr-click', (e) => {
      $('#global-search-value').val();
      const t = $(e.currentTarget).closest('.global-result').attr('data-url');
      t && (window.location = t);
    });
});
