        ...
        data: {
          ...
          .done(function (msg) {
            showNotification(
              ...
              'success',
              false,
              '/admin/settings/discount/edit/' + msg.discountId,
        })
          .fail(function (msg) {
          showNotification(msg.responseJSON.message, 'danger');
          })
...
function globalSearch() {
  ...
  $.ajax({
    ...
    res.orders.forEach((value) => {
      hasResult = true;
      const result = `
            <li class="list-group-item global-result text-center" data-url="/admin/order/view/${
  value._id
}">
                <div class="row">
                    <div class="col global-result-type gr-click"><i class="feather" data-feather="package"></i> Order</div>
                    <div class="col global-result-detail gr-click">${value.orderFirstname} ${
  value.orderLastname
}</div>
                    <div class="col global-result-detail gr-click">${moment(value.orderDate).format(
    'YYYY/MM/DD'
  )}</div>
      ...
      const result =
            `<li class="list-group-item global-result text-center" data-url="/admin/product/edit/${value._id}">
                <div class="row">
                    ...
                    <div class="col global-result-detail gr-click">${$(
    '#currencySymbol'
  ).val()}${numeral(value.productPrice).format('0.00')}</div>
                </div>
            </li>`;
      $('#global-search-results').append(result)
    })
...
function validationErrors(errors) {
  ...
  errors.forEach((value) => {
    errorMessage += `<p>${value.dataPath.replace('/', '')} - <span class="text-danger">${
      value.message
    }<span></p>`;
  })
  return errorMessage;
